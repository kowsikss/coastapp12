...
const HomePage = () => {
  ...
  const handleCapture = () => {
    ...
    // Call the handleImageCapture function with the captured image
  ...
  const handleImageCapture = async (imageSrc) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/detect-object", formData, {
      ...
      // ✅ Ensure objects are converted into a displayable format
      if (detectedObjects.length > 0) {
        setObjectInfo(detectedObjects.map(obj => 
          `Detected: ${obj.name} (Confidence: ${obj.confidence.toFixed(2)})`
        ));
      } else {
        setObjectInfo(["No objects detected."]);
      }
    } catch (error) {
